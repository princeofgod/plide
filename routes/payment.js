...
// 	const request = await axios.post('http://127.0.0.1:3002/payment/savePayments', data).then((response, err) => {
// 		if (err){
//             console.log (err)
//         }
//         console.log("newwwwwww==", response)
// })
})

router.get('/getPayments', async (req, res) => {
    const payments = Payment.find({}, (err, pay) => {
        if (err){
            console.log(err)
        }
        return pay;
